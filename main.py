...
class getCCB():
    ...
    def getApi(self, functionId, activityId='lPYNjdmN', prefix='jxjkhd', params=()):
        ...
        GET请求接口
        '''
        url = 'https://{}.kerlala.com/{}/91/{}'.format(
            prefix, functionId, activityId)
        headers = {
    ...
    def postApi(self, functionId, data, activityId='lPYNjdmN', prefix='jxjkhd'):
        '''
        POST请求接口
        '''
        url = 'https://{}.kerlala.com/{}/91/{}'.format(
    ...
    def getUserInfo(self):
        ...
        if userInfo != False:
            ...
            print('已获得CC币总量{}，剩余CC币总量{}'.format(
                userInfo['data']['ccb_money'], userInfo['data']['remain_ccb_money']))
            print('当前建筑等级{}级，已获得建设值总量{},升级还需建设值{}'.format(
                userInfo['data']['grade'], userInfo['data']['build_score'], userInfo['data']['need_build_score']))
    ...
    def acceptCCB(self):
        ...
        for i in range(len(ccbList['data'])):
            data = '{"id":"' + str(ccbList['data'][i]['task_id']) + \
                '","result_id": "' + str(ccbList['data'][i]['id']) + '"}'
            if ccbList['data'][i]['type'] == 'dailyCcb':
                acceptCcbResult = self.postApi(
                    'activity/fbtopic/acceptCcb', data)
    ...
    def doTask(self):
        ...
        if self.currentTime < activityInfo['data']['end_time']:
            ...
            for i in range(len(taskList['data']['task'])):
                print('\n开始做任务{}【{}】'.format(
                    i+1, taskList['data']['task'][i]['show_set']['name']))
                # 判断任务状态
                if taskList['data']['userTask'][i]['finish'] == 1:
                    ...
                    if taskList['data']['task'][i]['draw_type'] == 'number':
                        ...
                        data = '{"id":"' + \
                            taskList['data']['task'][i]['id'] + '"}'
                        acceptResult = self.postApi(
                            'Component/task/draw', data)
    ...
    def buildingUp(self):
        ...
        if userInfo['data']['remainder_build_score'] >= userInfo['data']['next_grade_build_score']:
            ...
            if len(buildingUpResult['data']['up_awards']['up_awards']) > 0:
                ...
                print('升级{}成功，获得奖励{}'.format(
                    buildingUpResult['data']['up_building']['name'], upAwardsName))
            else:
                print('升级{}成功，无奖励'.format(
                    buildingUpResult['data']['up_building']['name']))
            # 继续检查是否能升级
            time.sleep(5)
            self.buildingUp()
        else:
            print('建设值不足,距下一等级还需{}建设值'.format(
    ...
    def doCarTask(self):
        ...
        activityInfo = self.getApi(
            'Common/activity/getActivityInfo', 'dmRe4rPD')
        if self.currentTime < activityInfo['data']['end_time']:
            # 访问首页，获得三次抽奖机会
            ...
            for i in range(len(taskList['data']['task'])):
                print('\n开始做任务【{}】'.format(
                    taskList['data']['task'][i]['indicator']['show_name']))
                if taskList['data']['task'][i]['day_complete'] == 1:
                    print('该任务今日已完成，无需重复执行')
                elif taskList['data']['task'][i]['day_complete'] == 0:
                    data = '{"code": "' + \
                        taskList['data']['task'][i]['indicator']['code'] + '"}'
                    doTaskResult = self.postApi(
                        'activity/parallelsessions/visit', data, 'dmRe4rPD')
                    print(doTaskResult['message'])
            # 获取抽奖次数
            carIndexInfo = self.getApi(
                'activity/parallelsessions/index', 'dmRe4rPD')
            ...
            if int(carIndexInfo['data']['remain_num']) > 0:
                for i in range(int(carIndexInfo['data']['remain_num'])):
                    drawResult = self.postApi(
                        'activity/parallelsessions/draw', {}, 'dmRe4rPD')
    ...
    def draw(self):
        ...
        if self.currentTime < activityInfo['data']['end_time']:
            ...
            if drawStatus['status'] == 'fail':
                ...
                getUserCCBResult = self.getApi(
                    'Component/draw/getUserCCB', '03ljx6mW')
                userRemainDrawNum = getUserCCBResult['data']['draw_day_max_num'] - int(
                    getUserCCBResult['data']['user_day_draw_num'])
                if int(getUserCCBResult['data']['user_day_draw_num']) <= 3:
                    ...
                    for i in range(userRemainDrawNum):
                        drawResult = self.postApi(
                            'Component/draw/commonCcbDrawPrize', {}, '03ljx6mW')
                        if drawResult['data']['prize_type'] == 'jccb':
                            print(drawResult['message'] +
                                  drawResult['data']['prizename'])
                            prizeNum = int(re.findall(
                                '[0-9]*', drawResult['data']['prizename'])[0]) - 30
                    ...
                    print('今日已进行过{}次抽奖了,若有剩余次数可手动执行'.format(
                        getUserCCBResult['data']['user_day_draw_num']))
        else:
            print('抱歉，该活动已结束')
    ...
    def dayAnswer(self):
        ...
        print('\n开始每日一答')
        activityInfo = self.getApi(
            'Common/activity/getActivityInfo', 'jmX0aKmd')
        if self.currentTime < activityInfo['data']['end_time']:
            # 获取用户答题信息
            userDataInfo = self.getApi(
                'activity/dopanswer/getUserDataInfo', 'jmX0aKmd')
            if userDataInfo['data']['remain_num'] == 1:
                # 获取题目
                question = self.getApi(
                    'activity/dopanswer/getQuestion', 'jmX0aKmd')
                ...
                for i in range(len(question['data']['all_question'][0]['option'])):
                    print('选项{}：{}'.format(
                        i+1, question['data']['all_question'][0]['option'][i]['title']))
                # 随机答题
                randomOption = random.randint(
                    1, len(question['data']['all_question'][0]['option']))
                print('随机选择选项{}'.format(randomOption))
                data = '{"answerId":"' + str(randomOption) + \
                    '","questionId":"' + str(questionId) + '"}'
                answerQuestionResult = self.postApi(
                    'activity/dopanswer/answerQuestion', data, 'jmX0aKmd')
                print('正确答案：选项{}'.format(
                    answerQuestionResult['data']['right']))
    ...
    def mothersDayTask(self):
        ...
        print('\n开始做 母亲节集赞得520CC币 活动')
        activityInfo = self.getApi(
            'Common/activity/getActivityInfo', 'jmX08Ymd')
        if self.currentTime < activityInfo['data']['end_time']:
            # 获取用户信息
            userInfo = self.getApi('activity/mumbit/getUserInfo', 'jmX08Ymd')
            print('您的活动助力码为：{}'.format(userInfo['data']['ident']))
            judgeStatus = self.getApi(
                'activity/mumbit/judgeStatus', 'jmX08Ymd')
            if judgeStatus['data']['ext'] == '':
                print('您从未参加该活动，开始初始化活动')
                # 设置标签
                data = '{"ext":"[{\\"title\\":\\"\u89C9\u5F97\u6211\u6C38\u8FDC\u5403\u4E0D\u9971\\",\\"thumbnail_url\\":\\"https://ccbhdimg.kerlala.com/hd/users/10461/20210430/8645701907.png\\",\\"isChoosed\\":true,\\"left\\":\\"0rem\\",\\"top\\":\\"2.6rem\\"}]"}'.encode(
                    "utf-8").decode("latin1")
                updatePhraseResult = self.postApi(
            ...
            if userLaunchInfo['code'] == 2101:
                ...
                doUserLaunchResult = self.postApi(
                    'activity/mumbit/doUserLaunch', {}, 'jmX08Ymd',)
                print('开启分享结果：'.format(doUserLaunchResult['message']))
            print('当前账号已获赞{}，还需{}个点赞'.format(
                judgeStatus['data']['help_num'], judgeStatus['data']['need_help_num']))

            # 领取满助力奖励
            if int(judgeStatus['data']['help_num']) >= 20:
                print('已达到领奖条件，开始领取奖励')
                getRewardResult = self.getApi(
                    'activity/mumbit/draw', 'jmX08Ymd')
            ...
            for i in range(len(self.motherDayShareCode)):
                ...
                if userLaunchInfo['code'] == 2101:
                    ...
                    data = '{"launch_id":"' + \
                        str(launchId) + '","share_ident":"' + \
                        self.motherDayShareCode[i] + '"}'
                    doUserHelpResult = self.postApi(
                        'activity/mumbit/doUserHelp', data, 'jmX08Ymd')
                    print('助力好友{}结果：{}'.format(
                        friendName, doUserHelpResult['message']))
                    # 休息5秒，防止接口频繁
                    time.sleep(5)
        else:
            print('抱歉，该活动已结束')

    def doIndicatorTask(self):
        '''
        龙支付优惠锦集
        '''
        print('\n开始龙支付优惠锦集')
        ...
        if self.currentTime < activityInfo['data']['end_time']:
            # 获取任务列表
            taskList = self.getApi(
                'activity/lzfsubvenue/getIndicatorList', activityId='5Z9WxaPK', prefix='jxjkhd6')
            print('共获取到{}个任务'.format(len(taskList['data']['task'])))
            for i in range(len(taskList['data']['task'])):
                print('\n开始做任务{}【{}】'.format(
                    i+1, taskList['data']['task'][i]['indicator']['name']))
                # 判断任务状态
                if taskList['data']['task'][i]['day_complete'] == 1:
                    print('该任务已完成，无需重复执行')
                else:
                    ...
                    data = '{"code": "' + \
                        taskList['data']['task'][i]['indicator']['code'] + '"}'
                    doTaskResult = self.postApi(
                        'activity/lzfsubvenue/visit', data=data, activityId='5Z9WxaPK', prefix='jxjkhd6')
                    print(doTaskResult['message'])
                    # 休息五秒，防止接口提示频繁
                    time.sleep(5)
        else:
...
if __name__ == '__main__':
    ...
    configPath = rootDir + "/config.sample.json"
